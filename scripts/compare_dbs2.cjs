...
async function run() {
  console.log('Fetching stats for Database 1 (safestories_db)...');
  const db1 = await getStats(db1Config);
  
  console.log('Fetching stats for Database 2 (safestories_v2)...');
  const db2 = await getStats(db2Config);
  
  let report = '# Database Comparison Report (`safestories_db` vs `safestories_v2`)\n\n';
  report += 'Comparing **Database 1** (`safestories_db`) to **Database 2** (`safestories_v2`).\n\n';
  ...
  if (missingTablesIn1.length > 0) {
    report += '### Tables missing in Database 1 (`safestories_db`):\n';
  ...
  if (missingTablesIn2.length > 0) {
    report += '### Tables missing in Database 2 (`safestories_v2`):\n';
  ...
  report += '| Table Name | DB 1 (`safestories_db`) | DB 2 (`safestories_v2`) | Difference |\n';
  report += '|---|---|---|---|\n';
  ...
  fs.writeFileSync('db_comparison_output2.md', report);
  console.log('Report generated at db_comparison_output2.md');
}
