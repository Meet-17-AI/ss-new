...
async function run() {
  try {
    ...
    const charges = '₹';
    const slugBase = '/free-consultation-safestories-test2';
    ...
    const result = await pool.query(`
      ...
      RETURNING *
    `, [
      title, duration, type, therapy_type, description, charges, slugBase, therapist_id, therapist_name,
      payment_gateway, schedule_id, JSON.stringify(form_questions), requires_tnc, is_payment_enabled
    ]);
