...
const regex = /(\/\/ If it's a Free Consultation and we have a phone, create a new lead automatically\s+)(return res\.status\(400\)\.json\(\{ error: 'Missing required fields: clientName is required' \}\);)/;

const replaceStr = `$1if (isFreeConsultation && inviteePhone) {
